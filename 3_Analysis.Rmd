...
forest(model.post.IO_t2,
       slab = df$article,
       order = order(df$article, desc(df$effectLabel)))
```
...
out <- rbind(out,
             tibble(
               ...
               est = NA, #model.post.IO_t2$b[1,1],
               lower = NA, #model.post.IO_t2$ci.lb,
               upper = NA, #model.post.IO_t2$ci.ub
               n = NA
             ))
...
ggplot(out, 
       aes(y = effectLabel, 
           ...
           xmin = lower,
           xmax = upper)) +
  geom_point(aes(size = n), shape = "square") +
  geom_errorbarh() +
  ...
  geom_polygon(data = diamond, inherit.aes = F,
               aes(x = x, y = y))
```
...
```{r}
forest(model.fu.IO_t2,
       slab = df.fu$article,
       order = order(df.fu$article, desc(df.fu$effectLabel)))
```
...
out <- rbind(out,
             tibble(
               article = "Overall Estimate",
               effectLabel = "Overall Estimate",
               est = NA, #model.fu.IO_t2$b[1,1],
               lower = NA, #model.fu.IO_t2$ci.lb,
               upper = NA, #model.fu.IO_t2$ci.ub
               n = NA
             ))
