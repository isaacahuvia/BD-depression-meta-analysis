...
We will assess publication bias with a funnel plot and Egger test (using regtest in R). Since we cannot use `regtest` with `rma.mv`, we will instead use the SE of each effect size as a moderator and test that model.

```{r}
rma.mv(yi = g.post_t2 ~ 1 + sqrt(varG.post_t2),
       V = varG.post_t2,
       random = ~ 1 | studyID,
       data = df) %>%
  robust(cluster = df$studyID)
...
In addition, we will test the following as moderators:

- Blind assignment to study group
...
- Presence of pre-intervention training
...
rma.mv(yi = g.post_t2 ~ 1 + supervision,
       V = varG.post_t2,
       random = ~ 1 | studyID,
       data = df) %>%
  robust(cluster = df$studyID)
...
```{r}
rma.mv(yi = g.post_t2 ~ 1 + studyCompletion.post,
       V = varG.post_t2,
       random = ~ 1 | studyID,
       data = df) %>%
  robust(cluster = df$studyID)
...
df %>%
  ggplot(aes(g.post_t2, sqrt(varG.post_t2))) +
  geom_point() +
...
See for minimum n/group in moderator test: <https://www.researchgate.net/post/Does-anyone-have-any-citations-supporting-a-specific-number-of-studies-eg-3-or-more-per-subgroup-in-a-categorical-moderator-analysis>

Consider decomposing effects of effect-level and study-level moderation (see page 13, <https://cran.r-project.org/web/packages/robumeta/vignettes/robumetaVignette.pdf>)
...
```{r}
moderators <- c("year", "pubStatus", "country", "sampleSource", "meanAge", "pctFemale", "pctNonwhite", "interventionType", "riskScreen", "outcome", "controlCond", "deliveryFormat", "target", "provider", "supervision", "length.minutes", "length.sessions", "length.days", "trtCompletion", "studyCompletion.post", "interventionParticipant", "meanType", "randUnits")
...
Pre-registered moderators: `year`, `pubStatus`, `country`, `sampleSource`, `meanAge`, `pctFemale`, `pctNonwhite`, `interventionType`, `riskScreen`, `outcome`, `controlCond`, `deliveryFormat`, `target`, `provider`, `supervision`, `length.minutes`, `length.sessions`, `length.days`, `trtCompletion`, `studyCompletion.post`, `followUpLength`
...
```{r}
moderators <- c("year", "pubStatus", "country", "sampleSource", "meanAge", "pctFemale", "pctNonwhite", "interventionType", "riskScreen", "outcome", "controlCond", "deliveryFormat", "target", "provider", "supervision", "length.minutes", "length.sessions", "length.days", "trtCompletion", "studyCompletion", "interventionParticipant", "meanType", "followUpLength", "randUnits")
