...
and Johnson, 2010) and small-sample corrected (Tipton, 2013) RVE methods. Here "correlated" and "hierarchical" appear to refer to weighting schemes. This is a _correlated effects model_.
...
Mixed-effects model with robust variance estimation using `metafor`. This is a _mixed effects (aka random effects aka hierarchical) model_.
...
model.m.mod <- rma.mv(yi = g.post_t2 ~ 1 + year,
                      V = varG.post_t2,
                      random = ~ 1 | studyID,
                      data = df) %>%
  robust(cluster = df$studyID)
